...
function kg_application_details_box( $post ) {
    ...
    <table style="width:100%;border-collapse:collapse;">
        ...
        <tr>
            ...
            <td style="padding:10px 8px;border-bottom:1px solid #f0f0f0;">
                <span style="text-transform:capitalize;font-weight:bold;"><?php echo esc_html( $purpose ?: 'Job Application' ); ?></span>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td style="padding:10px 8px;border-bottom:1px solid #f0f0f0;">
                <?php if ( $street || $barangay || $city || $region ) : ?>
                    <?php if ($street) echo esc_html($street) . '<br>'; ?>
                    <?php if ($barangay) echo 'Brgy. ' . esc_html($barangay) . '<br>'; ?>
                    <?php if ($city) echo esc_html($city) . '<br>'; ?>
        ...
        <tr>
            ...
            <td style="padding:10px 8px;border-bottom:1px solid #f0f0f0;">
                ...
                <p class="description" style="margin: 4px 0 0 0; font-size: 11px; color: #666;">Select the matching job post that fits this applicant's profile.</p>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td style="padding:10px 8px;border-bottom:1px solid #f0f0f0;">
                ...
                ?>
                    <a href="<?php echo esc_url( $preview_url ); ?>" target="_blank" class="button button-primary" style="margin-right:8px;">👁 View CV</a>
                    <a href="<?php echo esc_url( $download_url ); ?>" target="_blank" class="button">⬇ Download CV</a>
                <?php else : ?>—<?php endif; ?>
            </td>
        </tr>
        <tr>
            <td style="padding:10px 8px;font-weight:600;vertical-align:top;border-bottom:1px solid #f0f0f0;">Staff Notes & Comments</td>
            <td style="padding:10px 8px;border-bottom:1px solid #f0f0f0;">
                <?php
                ...
                ?>
                <div class="kg-notes-thread" style="max-height: 250px; overflow-y: auto; margin-bottom: 12px; background: #f8fafc; padding: 10px; border-radius: 6px; border: 1px solid #e2e8f0;">
                    <?php if ( empty( $notes ) ) : ?>
                    ...
                    <?php else : ?>
                        <?php foreach ( array_reverse($notes) as $note ) : ?>
                            <div class="kg-note-item" style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #e2e8f0; font-size: 13px;">
                                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                                    <strong style="color: #1e293b;"><?php echo esc_html( $note['author'] ); ?></strong>
                                    <span style="color: #64748b; font-size: 11px;"><?php echo esc_html( date_i18n( get_option('date_format') . ' ' . get_option('time_format'), $note['timestamp'] ) ); ?></span>
                                </div>
                                <div style="color: #334155; line-height: 1.4; white-space: pre-wrap;"><?php echo esc_html( $note['message'] ); ?></div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
                <textarea name="kg_add_note" style="width: 100%; height: 70px; padding: 6px 10px; border-radius: 4px; border: 1px solid #ccc; font-size: 13px;" placeholder="Type a note here to add to the thread..."></textarea>
                <p class="description" style="margin: 4px 0 0 0; font-size: 11px; color: #666;">Type a note and click "Update" above or below to save it.</p>
            </td>
        </tr>
        <tr>
            ...
            <td style="padding:10px 8px;">
                ...
                if ( ! $history_query->have_posts() ) : ?>
                    <p style="color: #64748b; font-style: italic; margin: 0; font-size: 13px;">No prior applications found for this applicant.</p>
                <?php else : ?>
                        ...
                        <ul style="margin: 0; padding-left: 15px; line-height: 1.5;">
                            ...
                            ?>
                                <li style="margin-bottom: 6px;">
                                    ...
                                    <span style="font-size: 11px; padding: 2px 6px; border-radius: 4px; font-weight: bold; margin-left: 5px; background: #e2e8f0; color: #475569;"><?php echo esc_html( $h_status_lbl ); ?></span>
                                    <a href="<?php echo esc_url( get_edit_post_link( $h_id ) ); ?>" style="margin-left: 8px; text-decoration: none;">View</a>
                                </li>
...
function kg_application_status_box( $post ) {
    ...
    <div style="margin-bottom:12px;">
        ...
        <select name="kg_app_status" data-original-status="<?php echo esc_attr( $status ); ?>" style="width:100%;padding:8px;font-size:14px;">
    ...
    <div style="margin-top:16px; border-top:1px solid #eee; padding-top:12px; display: <?php echo in_array( $status, array( 'screening', 'interviewing' ), true ) ? 'block' : 'none'; ?>;" id="kg-scheduler-section">
        ...
        <div style="margin-bottom:8px;">
            ...
            <input type="date" name="kg_interview_date" value="<?php echo esc_attr($int_date); ?>" style="width:100%; padding:4px;" />
        </div>
        
        <div style="margin-bottom:8px;">
            <label style="font-size:11px;font-weight:600;display:block;margin-bottom:2px;">Time</label>
            <input type="time" name="kg_interview_time" value="<?php echo esc_attr($int_time); ?>" style="width:100%; padding:4px;" />
        </div>
        ...
        <div style="margin-bottom:8px;">
            ...
            <textarea name="kg_interview_details" style="width:100%; height:45px; padding:4px; font-size:12px;"><?php echo esc_textarea($int_details); ?></textarea>
        </div>
        ...
        <div style="margin-bottom:8px;">
            ...
            <select name="kg_interviewer_id" style="width:100%; padding:4px;">
                ...
                <?php foreach ($recruiters as $rec) : ?>
                    <option value="<?php echo esc_attr($rec->ID); ?>" <?php selected($int_er_id, $rec->ID); ?>><?php echo esc_html($rec->display_name); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
    ...
    <div style="margin-top:16px; border-top:1px solid #eee; padding-top:12px; display: <?php echo ( $status === 'deployed' ) ? 'block' : 'none'; ?>;" id="kg-deploy-date-section">
        ...
        <div style="margin-bottom:8px;">
            ...
            <input type="date" name="kg_app_deploy_date" value="<?php echo esc_attr( get_post_meta( $post->ID, 'kg_app_deploy_date', true ) ); ?>" style="width:100%; padding:4px;" />
        </div>
    </div>

    <p style="font-size:12px;color:#666;margin:10px 0 0 0;">Click <strong>Update</strong> to save pipeline &amp; scheduling changes.</p>
...
function kg_notify_applicant_status( $post_id, $status ) {
    ...
    if ( $barangay ) $address_parts[] = 'Brgy. ' . $barangay;
...
function kg_application_admin_scripts() {
    ...
    if ( $pagenow !== 'edit.php' || $post_type !== 'kg_application' ) return;
    ?>
    <style>
...
function kg_filter_applications_for_recruiters($query) {
    ...
    if (is_admin() && $query->is_main_query() && $pagenow === 'edit.php' && $query->get('post_type') === 'kg_application') {
        if (kg_is_current_user_recruiter()) {
            ...
            // Scoping meta query:
            // 1. Explicitly assigned to this recruiter ID, OR
            // 2. Matching this recruiter's location, OR
            // 3. Application has no location metadata yet (fallback)
            $scope_query = array(
                'relation' => 'OR',
                array(
            ...
            // Fallback for unassigned/old applications
            $scope_query[] = array(
                'key'     => 'kg_app_location',
                'value'   => '',
                'compare' => 'NOT EXISTS',
        ...
        if ( isset( $_GET['kg_trigger_bulk_assign'] ) && ! empty( $_GET['ids'] ) ) {
            ...
            <div class="notice notice-info is-dismissible" style="padding:15px;">
                ...
                <form method="post" action="">
                    ...
                    <a href="<?php echo esc_url( remove_query_arg( array( 'kg_trigger_bulk_assign', 'ids' ) ) ); ?>" class="button button-link">Cancel</a>
                </form>
            </div>
            <?php
        }
...
function kg_email_templates_settings_render() {
    ...
    <div class="wrap">
        ...
        <p>Customize the automated emails sent by the recruitment and sales systems. Use the left menu to navigate through templates.</p>
        ...
        <div style="display:flex; gap:20px; align-items:flex-start;">
            ...
            <div style="width:250px; background:#fff; border:1px solid #ccd0d4; box-shadow:0 1px 1px rgba(0,0,0,.04);">
                ...
                foreach ( $groups as $group_key => $group ) : ?>
                    <div style="background:#f0f0f1; padding:10px 12px; font-weight:700; font-size:11px; text-transform:uppercase; letter-spacing: 0.05em; color:#50575e; border-bottom:1px solid #ccd0d4; border-top:<?php echo $group_key === 'branding' ? '0' : '1px solid #ccd0d4'; ?>;">
                    ...
                    <ul style="margin:0; padding:0; list-style:none;">
                        <?php foreach ( $group['keys'] as $k ) : 
                            ...
                            <li style="margin:0; border-bottom:1px solid #ccd0d4;">
                                <a href="<?php echo esc_url(add_query_arg('template', $k)); ?>" style="display:block; padding:10px 16px; text-decoration:none; font-weight:600; font-size:12px; background:<?php echo $bg; ?>; color:<?php echo $color; ?>;" <?php echo $hover_style; ?>>
            ...
            <!-- Right Content Form -->
            <div style="flex:1; background:#fff; border:1px solid #ccd0d4; padding:20px; box-shadow:0 1px 1px rgba(0,0,0,.04);">
                <h2>Edit Template: <?php echo esc_html($active['label']); ?></h2>
                <p style="color:#64748b; font-style:italic; margin-bottom:20px;"><?php echo esc_html($active['desc']); ?></p>
                    ...
                    <?php if ( $active_key === 'global_branding' ) : ?>
                        <table class="form-table" role="presentation">
                            <tr>
                                ...
                                <td>
                                    <div id="logo_preview_container" style="margin-bottom: 10px; <?php echo empty($branding_data['logo_url']) ? 'display:none;' : ''; ?>">
                                        <img id="logo_preview_img" src="<?php echo esc_url($branding_data['logo_url'] ?? ''); ?>" style="max-height: 80px; background: #f0f0f1; padding: 10px; border-radius: 4px; display: block;" />
                                    </div>
                                    <input type="text" name="logo_url" id="logo_url" value="<?php echo esc_url($branding_data['logo_url'] ?? ''); ?>" class="large-text" readonly style="background: #f0f0f1; cursor: not-allowed; margin-bottom: 10px;" />
                                    <div>
                                        <button type="button" class="button button-secondary" id="kg_upload_logo_btn">Select or Upload Logo</button>
                                        <button type="button" class="button button-link-delete" id="kg_remove_logo_btn" style="margin-left: 10px; <?php echo empty($branding_data['logo_url']) ? 'display:none;' : ''; ?>">Remove Logo</button>
                                    </div>
                                    <p class="description">Select or upload the logo image (preferably PNG/WebP with transparent bg) from the Media Library. It will be centered at the top of your emails.</p>
                            ...
                            <tr>
                                ...
                                <td>
                                    <textarea name="footer_address" id="footer_address" class="large-text" rows="3" placeholder="100 Doña Soledad Ave, Better Living, Parañaque, 1711 Philippines"><?php echo esc_textarea($branding_data['footer_address'] ?? '100 Doña Soledad Ave, Better Living, Parañaque, 1711 Philippines'); ?></textarea>
                                    <p class="description">Physical corporate details rendered in the email footer signature block.</p>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="footer_phone">Footer Phone / Contacts</label></th>
                                <td>
                                    <input type="text" name="footer_phone" id="footer_phone" value="<?php echo esc_attr($branding_data['footer_phone'] ?? '+63 2 8-776-6712 | +63 2 7-738-8071'); ?>" class="large-text" placeholder="+63 2 8-776-6712 | +63 2 7-738-8071" />
                                    <p class="description">Display phone details printed at the bottom of standard transactional emails.</p>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="footer_web_text">Footer Website Link Text</label></th>
                                <td>
                                    <input type="text" name="footer_web_text" id="footer_web_text" value="<?php echo esc_attr($branding_data['footer_web_text'] ?? 'kingsgroup.com.ph'); ?>" class="regular-text" placeholder="kingsgroup.com.ph" />
                                    <p class="description">Branded target link label displayed in the email footer. Defaults to kingsgroup.com.ph.</p>
                                </td>
                            </tr>
                        </table>
                    <?php else : ?>
                        <table class="form-table" role="presentation">
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="subject" id="subject" value="<?php echo esc_attr($subj); ?>" class="large-text" required />
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="heading">Email Heading</label></th>
                                <td>
                                    <input type="text" name="heading" id="heading" value="<?php echo esc_attr($heading); ?>" class="large-text" required />
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <p class="description" style="margin-top:8px;">
                                        <strong>Supported tokens:</strong> <code><?php echo esc_html(implode(', ', $active['tokens'])); ?></code>
                                    </p>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="banner" id="banner" value="<?php echo esc_attr($banner); ?>" class="large-text" />
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="btn_text" id="btn_text" value="<?php echo esc_attr($btn_t); ?>" class="regular-text" />
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="btn_link" id="btn_link" value="<?php echo esc_attr($btn_l); ?>" class="large-text" />
                                    <p class="description">Supports link URLs or dynamic tokens like <code>{site_url}</code> or <code>{edit_url}</code>.</p>
                                </td>
                            </tr>
                        </table>
                    <?php endif; ?>
