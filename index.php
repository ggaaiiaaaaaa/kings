...
$page_hero_bg = kg_get_field('news_bg', kg_asset('img/community/hero-community.png'), $posts_page_id);
get_header();

// Determine if we are on the blog archive or a general archive
$is_news_archive = is_home() || is_archive();
$title = $is_news_archive ? kg_get_field('news_headline', 'News & Updates', $posts_page_id) : 'Kings Group';
$subtitle = $is_news_archive ? kg_get_field('news_desc', 'Stay informed with the latest insights, company news, and industry updates from the Kings Group community.', $posts_page_id) : 'Elite talent acquisition and ethical staffing solutions.';
?>

<!-- Hero Section -->
<section class="page-hero" style="background-image: linear-gradient(rgba(10, 37, 64, 0.8), rgba(10, 37, 64, 0.8)), url('<?php echo esc_url($page_hero_bg); ?>');">
    <div class="container text-center">
        <h1 class="animate-on-scroll"><?php echo esc_html($title); ?></h1>
        <p class="animate-on-scroll" style="max-width: 700px; margin: 0 auto; font-size: 1.5rem;"><?php echo esc_html($subtitle); ?></p>
    </div>
</section>
...
<main id="main-content" class="news-page-root">
    <div class="container news-page-container">
        ...
        <?php
        // Fetch featured post (latest post)
        $featured_post = null;
        if ( have_posts() && get_query_var('paged', 1) <= 1 ) {
            the_post();
        ...
        <section class="news-featured-showcase animate-on-scroll">
            <a href="<?php echo get_permalink($featured_post); ?>" class="featured-link-wrapper">
                ...
                <div class="featured-details-col">
                    <span class="featured-tag">Latest Story</span>
        ...
        <section class="older-news-section">
            <?php if ( have_posts() ) : ?>
                <h2 class="section-subtitle">More Stories</h2>
